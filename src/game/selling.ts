...
export function sellItem(stateRef: { value: GameState }, item: Item): void {
  const bonusUpgrade = stateRef.value.upgrades.find((u) => u.category === 'gold_bonus')
  ...
  let statIdx = stateRef.value.stats.sales.findIndex((i) => i.item_id === item.item_id)
  if (statIdx === -1) {
...
export function userDidSell(stateRef: { value: GameState }, item: Item, sellAll = false): void {
  if (sellAll) {
...
export function userDidSellAll(stateRef: { value: GameState }, item: Item): void {
  const invItem = stateRef.value.inventory.find((i) => i.item_id === item.item_id)
  if (!invItem) return
...
export function assistantDidSell(
  ...
  const itemConfig = assistant.config.selling.find((c) => c.item_id === item.item_id)
  ...
  switch (itemConfig.method) {
    case 'sell_x':
      quantity = itemConfig.value > invItem.quantity ? invItem.quantity : itemConfig.value
      break
    case 'keep_x':
