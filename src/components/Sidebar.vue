...
const { s, upkeep, availableOreList, availableSmithableList, purchasedAutoerEntities, purchasedUpgradeEntities, autoerGroups } = storeToRefs(gameStore)
...
<template>
  <div id="sidebar">
    ...
    <div>
      ...
      <div class="levels">
        <div>
          ...
          <progress :value="gameStore.xpForSkill('mining')" max="100" :title="String(s.xp.mining)" />
        </div>
        <div>
    ...
    <TogglePanel
      ...
      <ul style="width: 75%; margin: 0 auto; padding-left: 0; list-style-type: none;">
    ...
    <TogglePanel
      ...
      <ul class="simple-list inventory" style="padding-left: 1rem;">
        <li v-for="category in ['ores', 'bars', 'armor', 'weapons']" :key="category">
          <span
            style="position: relative; display: block; margin-bottom: 0.75rem; padding: 0.25rem 0; font-size: 1.2rem; text-transform: capitalize; border-bottom: 1px solid black; cursor: pointer;"
            @click="uiStore.toggleInventory(category)">
            {{ category }}
            <span
              style="position: absolute; right: 0;"
              :style="{ transform: `rotate(${toggleState.inventory_categories[category] ? 0 : 180}deg)` }">
              ^
            </span>
          </span>
          <ul v-show="toggleState.inventory_categories[category]" style="padding-left: 0.75rem;">
            <li
              v-for="invItem in s.inventory.filter((i) => gameStore.getItem(i.item_id)?.categories.includes(category))"
              :key="invItem.item_id">
              <div>
                {{ gameStore.getItem(invItem.item_id)?.name }} x{{ invItem.quantity }}
                <div>
                  <button
                    type="button"
                    @click.shift="gameStore.userDidSell(gameStore.getItem(invItem.item_id)!, true)"
                    @click.exact="gameStore.userDidSell(gameStore.getItem(invItem.item_id)!)"
                    :disabled="!gameStore.hasInventory(gameStore.getItem(invItem.item_id)!)">
                    $
                  </button>
                  <button
                    type="button"
                    @click="gameStore.userDidSell(gameStore.getItem(invItem.item_id)!, true)"
                    :disabled="!gameStore.hasInventory(gameStore.getItem(invItem.item_id)!)"
                    class="mobile-only"
      ...
      <div style="margin-top: 1rem; text-align: right;">
        <button :title="`Bank Sale! ${gameStore.getBankSaleValue()}gp!`" @click="gameStore.bankSale()">
          Sell All
        </button>
      </div>
    </TogglePanel>
    ...
    <TogglePanel
      ...
      <ul class="simple-list ore-list">
        <li v-for="ore in availableOreList" :key="ore.id">
          <button class="skill-btn" type="button" @click="gameStore.userDidMine(ore)">
            {{ ore.name }}
          </button>
        </li>
      </ul>
      ...
      <ul class="simple-list smithing-list">
        <li v-for="(smithables, tier) in availableSmithableList" :key="tier">
          <span
            ...
            <span
              style="position: absolute; right: 0;"
              :style="{ transform: `rotate(${toggleState.inventory_categories[String(tier)] ? 0 : 180}deg)` }">
              ^
            </span>
          </span>
          <ul v-show="toggleState.inventory_categories[String(tier)]" style="margin-top: 0.5rem; padding-left: 0;">
    ...
    <TogglePanel
      ...
      <ul>
        ...
        <li
          ...
          <button type="button" @click="assistantStore.editAssistant(assistant)">Edit Job</button>
          <button type="button" @click="assistantStore.confirmFireAssistant(assistant)">Fire Assistant</button>
        </li>
      </ul>
    </TogglePanel>
    ...
    <TogglePanel
      ...
      <ul>
        ...
        <li v-for="autoerGroup in autoerGroups" :key="autoerGroup.id">
          ...
          <ul>
            <li
              v-for="autoer in purchasedAutoerEntities.filter((a) => a?.id === autoerGroup.id)"
