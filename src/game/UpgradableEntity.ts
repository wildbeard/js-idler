...
function buildSharedMethods(
  ...
  const getUpgradeFromState = () => {
    if (properties.category === 'autoer') {
      return stateRef.value.purchased_autoers.find((u) => u.unique_id === properties.unique_id)
    }
  ...
  const getUpgradeCost = (): number => {
    ...
    const maxLevel = properties.upgrades.sort((a, b) => a.level - b.level).at(-1)
    if (maxLevel?.level === currPurchased?.level) return -1
    const cost = properties.upgrades.find((u) => u.level === (currPurchased?.level ?? 0) + 1)
  ...
  const getUpgradeValueText = (nextLevel = false): string => {
    ...
    if (properties.category !== 'autoer') return getValueTextForUpgrade(nextLevel)
    ...
    if (autoer.xp_upgrade_ref) {
      const xpUpgradeState = stateRef.value.upgrades.find((u) => u.id === autoer.xp_upgrade_ref)
      if (xpUpgradeState) {
        const xpUpgradeData = upgradeData.find((u) => u.id === autoer.xp_upgrade_ref)
        const xpLevel = xpUpgradeData?.upgrades.find((u) => u.level === xpUpgradeState.level)
        if (xpLevel) {
...
export function createAutoerEntity(
  ...
  const getAutoerFunction = (): void => {
    ...
    if (properties.affects.includes('_weapons') || properties.affects.includes('_armor')) {
      const age = properties.affects.split('_')[0]
      const cat = properties.affects.split('_')[1]
      eligibleItems = items.filter((i) => i.categories.includes(cat) && i.categories.includes(age))
    ...
    const eligibleItem = eligibleItems[Math.floor(Math.random() * eligibleItems.length)]
  ...
  const toggleState = (preferredState?: 'on' | 'off'): void => {
    ...
    const stopAutoer = () => {
      clearInterval(
        stateRef.value.running_autoers[properties.unique_id] as ReturnType<typeof setInterval>,
      )
      stateRef.value.running_autoers[properties.unique_id] = null
    }

    if (!preferredState) {
      stateRef.value.running_autoers[properties.unique_id] ? stopAutoer() : startAutoer()
    } else if (preferredState === 'on' && !stateRef.value.running_autoers[properties.unique_id]) {
      startAutoer()
