...
export const useQuestStore = defineStore('quest', () => {
  ...
  if (gameStore.s.quests_started.length) {
    const q = questData.find(
      (q) => q.id === gameStore.s.quests_started[0].quest_id,
    );
    if (q) {
  ...
  function completeQuestStep() {
    ...
    completeQuestStepFn(currentQuest.value, currentStep, {
      value: gameStore.s,
    });
  ...
  return {
    ...
    updateQuestPanel,
    closeQuestPanel,
    canStartQuest: (quest: Quest) =>
      checkQuestRequirements(quest, { value: gameStore.s }),
    canCompleteQuest: (quest: Quest) =>
      canCompleteQuestFn(quest, { value: gameStore.s }),
    canCompleteQuestStep: (quest: Quest) => {
      if (!gameStore.s.quests_started[0]) return false;
      return canCompleteQuestStepFn(quest, gameStore.s.quests_started[0].step, {
        value: gameStore.s,
      });
    },
    ...
    getCurrentQuestStep: () =>
      gameStore.s.quests_started.length
        ? gameStore.s.quests_started[0].step
        : null,
  };
});
