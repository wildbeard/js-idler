...
import { createAutoerEntity, createUpgradeEntity, type AutoerEntity, type UpgradeEntity } from './UpgradableEntity'
...
export function loadState(): GameState {
  ...
  if (!loadedVer || Number(currVer[0]) > Number(loadedVer[0]) || Number(currVer[1]) > Number(loadedVer[1])) {
...
export function applyUpgrade(
  ...
  if (entity.category === 'autoer') {
    ...
    if (stateRef.value.running_autoers[autoerEntity.unique_id]) {
      clearInterval(stateRef.value.running_autoers[autoerEntity.unique_id] as ReturnType<typeof setInterval>)
    }
    ...
    const currIdx = list.findIndex((a) => a.unique_id === autoerEntity.unique_id)
    const record = { id: autoerEntity.id, unique_id: autoerEntity.unique_id, level: up.level }
    ...
    if (upgradeEntity.affects.includes('.')) {
      ...
      let target: Record<string, unknown> = stateRef.value as unknown as Record<string, unknown>
      while (path.length > 1) {
        const p = path.shift()!
...
export function hydrateState(loaded: GameState, stateRef: Ref<GameState>): void {
  ...
  if (loaded.purchased_autoers.length) {
    for (const a of loaded.purchased_autoers) {
      ...
      const entity = createAutoerEntity({ ...fromData, unique_id, level: a.level }, wrapper)
  ...
  if (loaded.upgrades.length) {
    for (const up of loaded.upgrades) {
      ...
      if (!fromData) continue
      const entity = createUpgradeEntity({ ...fromData, level: up.level }, wrapper)
  ...
  for (const key in startingState.global_variables) {
    ...
    if (!stateRef.value.global_variables[k]) {
      stateRef.value.global_variables[k] = startingState.global_variables[k] as never
    }
  }

  // Sanitize gold
  if (isNaN(stateRef.value.gold) || stateRef.value.gold < 0 || stateRef.value.gold == null) {
