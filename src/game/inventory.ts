import type { GameState, Item, StatEntry } from '../types'

export function updateInventory(stateRef: { value: GameState }, key: string, value: number): void {
...
export function getInventoryItem(stateRef: { value: GameState }, key: string): number {
...
export function updateStats(
  ...
  let idx = stateRef.value.stats[stat].findIndex((i) => i.item_id === item.item_id)
  if (idx === -1) {
