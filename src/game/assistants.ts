...
import { perksData, assistantFirstNames, assistantLastNames } from '../data/perks'
...
export function createPerkFromData(data: PerkData): Perk {
  ...
  return {
    ...
    id: data.id,
    name: data.name,
    description: data.description_template.replace('&', String(Math.floor(value * 100))),
    skills: data.skills,
...
export function generateRandomAssistant(
  ...
  const skill =
    preferredSkill ?? skills.sort(() => Math.random() - 0.5)[0]
...
export function hireAssistant(assistant: Assistant, stateRef: { value: GameState }): void {
  ...
  for (const skill of assistant.skills) {
    if (skill !== 'selling') {
      const avail = items.filter(
        (i) => i.skill === skill && i.level <= (stateRef.value.levels[skill] ?? 1),
      )
      hiredAssistant.config[skill as 'mining' | 'smithing'] = avail.map((i) => i.item_id)
    }
  }
...
export function fireAssistant(assistant: Assistant, stateRef: { value: GameState }): void {
  clearInterval(assistant.interval_id as ReturnType<typeof setInterval>)
  stateRef.value.assistants = stateRef.value.assistants.filter((a) => a.id !== assistant.id)
}
...
export function getAssistantJobFunctions(
  ...
  for (const key of ['mining', 'smithing', 'selling'] as const) {
    jobs.push(() => {
      ...
      switch (key) {
        case 'mining':
          actionableItems = resourceNodes.filter(
            (i) =>
              i.skill === 'mining' &&
              i.yields.filter((y) => assistant.config.mining.includes(y.item_id)).length > 0,
          )
          break
        case 'smithing':
        ...
        case 'selling':
          actionableItems = items.filter((i) => {
            const config = assistant.config.selling.find((c) => c.item_id === i.item_id)
            const invItem = stateRef.value.inventory.find((ii) => ii.item_id === i.item_id)
      ...
      switch (key) {
        case 'mining':
          assistantDidMine(stateRef, assistant, actionItem as typeof resourceNodes[0])
          break
        case 'smithing':
          if (hasIngredientsFor(stateRef, actionItem as typeof items[0])) {
            assistantDidSmith(stateRef, assistant, actionItem as typeof items[0])
          }
          break
        case 'selling':
          assistantDidSell(stateRef, assistant, actionItem as typeof items[0])
          break
      }
    })
  }
...
export function canHireAssistant(assistant: Assistant, stateRef: { value: GameState }): boolean {
  ...
  return stateRef.value.gold >= requirements.cost && hasRequirementsForAssistant(assistant, stateRef)
}

export function isAssistantMaxed(assistant: Assistant, stateRef: { value: GameState }): boolean {
  const currLvl = stateRef.value.assistants.find((a) => a.id === assistant.id)
  const nextLvl = currLvl ? currLvl.level + 1 : 0
...
export function upgradeAssistant(
  ...
  const perkTemplate = perksData.find((p) => p.id === purchasedAssistant.perk.id)
  if (perkTemplate) {
    purchasedAssistant.perk.description = perkTemplate.description_template.replace(
      '&',
