...
export function getUpkeep(stateRef: { value: GameState }): {
  assistant: number;
  progress: number;
} {
  const totalLevel =
    stateRef.value.levels.mining + stateRef.value.levels.smithing;
  ...
  const assistant = stateRef.value.assistants.reduce((u, a) => {
    return (u += Math.floor(
      Math.max(1, a.level) *
        stateRef.value.global_variables.base_assistant_upkeep +
        a.cost_per_action * Math.max(1, a.level) * 0.25,
    ));
  }, 0);

  const capitalismUpgrade = stateRef.value.upgrades.find(
    (u) => u.id === 'capitalism',
  );
  ...
  if (capitalismUpgrade) {
    ...
    capitalismRate =
      staticData?.upgrades.find((u) => u.level === capitalismUpgrade.level)
        ?.value ?? 0;
  }
...
export function taxManCometh(stateRef: { value: GameState }): void {
  ...
  stateRef.value.gold = Math.max(
    stateRef.value.gold - upkeep.assistant - upkeep.progress,
    0,
  );
}
...
export function getUpkeepDisplayTime(stateRef: { value: GameState }): string {
  ...
  const time =
    timeframe === 'minute'
      ? (interval / 1000 / 60).toFixed(1)
      : (interval / 1000).toFixed(2);
