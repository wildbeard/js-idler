...
const miningItems = computed(() =>
  items.filter((i) => i.skill === 'mining' && i.level <= s.value.levels.mining),
)
const smithingItems = computed(() =>
  items.filter((i) => i.skill === 'smithing' && i.level <= s.value.levels.smithing && i.sellable),
)
const sellingItems = computed(() =>
  items.filter((i) => i.level <= (s.value.levels[i.skill ?? 'mining'] ?? 1) && i.sellable),
)
</script>
...
<template>
  <div class="floating-window assistant-config">
    ...
    <p>{{ assistant.name }} will randomly perform one of the following tasks:</p>
          ...
          <li v-for="item in sellingItems" :key="item.item_id">
            ...
            <div
              ...
              <label style="flex: 0 0 100%;">
                <input type="radio" v-model="config.selling[item.item_id].method" value="all" />
                Sell All
              </label>
              <label style="flex: 0 0 100%;">
                <input type="radio" v-model="config.selling[item.item_id].method" value="sell_x" />
                Sell X
              </label>
              <label style="flex: 0 0 100%;">
                <input type="radio" v-model="config.selling[item.item_id].method" value="keep_x" />
                Keep X in Inventory
              </label>
              <label
                v-if="config.selling[item.item_id].method !== 'all'"
                style="flex: 0 0 calc(100% - 4px); margin-left: 4px;">
                {{ config.selling[item.item_id].method === 'sell_x' ? 'Sell' : 'Keep' }}
                <input
                  type="number"
    ...
    <div style="margin-top: 1rem; text-align: right;">
      <button style="margin-right: 15px;" @click="$emit('save', assistant)">Save</button>
