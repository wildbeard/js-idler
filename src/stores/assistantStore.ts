...
export const useAssistantStore = defineStore('assistant', () => {
  ...
  setInterval(() => {
    if (!gameStore.s.quests_completed.includes('introduction_assistants'))
      return;
    const assistant = generateRandomAssistant({ value: gameStore.s });
    if (
      availableAssistants.value.length >=
      gameStore.s.global_variables.max_available_assistants
    ) {
  ...
  const availableFiltered = computed(() =>
    availableAssistants.value.filter((a) =>
      hasRequirementsForAssistant(a, { value: gameStore.s }),
    ),
  );

  function hireAssistant(assistant: Assistant) {
    hireAssistantFn(assistant, { value: gameStore.s });
    availableAssistants.value = availableAssistants.value.filter(
      (a) => a.id !== assistant.id,
    );
    configuringAssistant.value =
      gameStore.s.assistants[gameStore.s.assistants.length - 1];
  }
  ...
  function saveAssistantConfig(purchasedAssistant: Assistant) {
    const idx = gameStore.s.assistants.findIndex(
    ...
    for (const key in configuringAssistantConfig.value) {
      if (key === 'selling') {
        ...
        const k = key as 'mining' | 'smithing';
        purchasedAssistant.config[k] = [
          ...(configuringAssistantConfig.value[k] ?? []),
        ];
      }
    }

    gameStore.s.assistants[idx] = JSON.parse(
  ...
  return {
    ...
    canHireAssistant: (assistant: Assistant) =>
      gameStore.s.global_variables.max_assistants >
        gameStore.s.assistants.length &&
    ...
    getAssistantUpgradeCost: (assistant: Assistant) =>
      assistant.upgrades.find((u) => u.level === assistant.level + 1)?.cost ??
      0,
    getAssistantUpgradeTitle: (assistant: Assistant) => {
      ...
      let str = 'Requires';
      for (const [k, v] of Object.entries(nxtLvl.requirements))
        str += ` ${k}: ${v}`;
      return str;
    },
    getAssistantJob: (assistant: Assistant) =>
      gameStore.s.assistants
        .find((a) => a.id === assistant.id)
        ?.skills.join(',') ?? 'N/A',
